function dxdt = integratingfunction_cytokine_treatment(t,x,params,Init)
% See https://www.mathworks.com/help/matlab/math/ode-event-location.html
% for more details.
%%%%%%%%%%%%%%%%%%%%
%
% Name: integratingfunction_cytokine_treatment.m
...
%
% Specifically, this function is used to simulate the cytokine experiments.
%
...
%--------------------------------------------------------------------------
%Cells:
%TN4=x(1); Th1=x(2); Th2=x(3); TN8=x(4); Tc=x(5);

%Proteins:
%IFN_gamma=x(6); IL-4=x(7); IL-6=x(8); IL-12=x(9); PD-1=x(10); PD-L1=x(11);
%PD-1:PD-L1=x(12)
...
dxdt(1) = n_4_tilde*x(1)...
        - (d_1_12_tilde*x(1) * (x(9)/(q_dIL12_tilde + x(9)))...
        +  d_1_IFN_tilde*x(1) * (x(6)/(q_IFN_1_tilde + x(6))))*(s_1_tilde/(s_1_tilde+x(12)))...
        - (d_2_tilde*x(1) * (x(7)/(q_dIL4_tilde + x(7))))*(s_2_tilde/(s_2_tilde+x(12)));
            
%(2) T_N8: 
dxdt(2) = n_8_tilde*x(2) - d_C_tilde*x(2) * (x(3)/(q_1+x(3)))*(s_C_tilde/(s_C_tilde + x(12)));
            
%(3) Th1: 
dxdt(3) = n_1_tilde*x(3) + (d_1_12_tilde*x(1) * (x(9)/(q_dIL12_tilde + x(9))) + ...
          d_1_IFN_tilde*x(1) * (x(6)/(q_IFN_1_tilde + x(6))))*(s_1_tilde/(s_1_tilde+x(12)));
            
%(4) Th2: 
dxdt(4) = (g_2_tilde*x(4) + g_2_4_tilde*x(4) * (x(7)/(q_gIL4_tilde + x(7)))) * ...
          (r_IFN_tilde/(r_IFN_tilde + x(6))) + ...
          (d_2_tilde*x(1) * (x(7)/(q_dIL4_tilde + x(7))))*(s_2_tilde/(s_2_tilde+x(12))) - ...
           delta_2_tilde*x(4);
            
%(5) Tc: 
dxdt(5) = n_C_tilde*x(5) + g_C_12_tilde*x(5)*x(9)/(q_gIL12_tilde + x(9))...
        + d_C_tilde*x(2) * (x(3)/(q_1+x(3)))*(s_C_tilde/(s_C_tilde + x(12)));

% Normalized to IFNg_0    
%(6) IFN_gamma: 
dxdt(6) = p_1_IFN_tilde*x(3) * (r_IL4_tilde/(r_IL4_tilde + x(7))) * (r_IL6_tilde/(r_IL6_tilde + x(8))) + ...
          p_C_IFN_tilde*x(5) - delta_IFN_tilde*x(6);
            
% Normalized to IL4_0
%(7) IL-4: 
dxdt(7) = p_2_4_tilde*x(4) + p_2_4_6_tilde*x(4) * (x(8)/(q_IL6_tilde + x(8))) - ...
          delta_IL4_tilde*x(7);
            
% Normalized to IL6_0
%(8) IL-6: 
dxdt(8) = p_2_6_tilde*x(4) - delta_IL6_tilde*x(8);

% Normalized to IL12_0            
%(9) IL-12: 
dxdt(9) = p_1_12_tilde*x(3)- delta_IL12_tilde*x(9);

% Normalized to PD1_0        
%(10) PD-1: 
dxdt(10) =  rho_tilde*(dxdt(3) + dxdt(4) + dxdt(5))...  
            - beta_plus_tilde*PDL1_0*x(10)*x(11) + beta_minus_tilde*x(12)...
            - alpha_plus_tilde*A_0*x(10)*x(13) + alpha_minus_tilde*A_0*x(14)/PD1_0;        
        
% Normalized to PDL1_0   
%(11) PD-L1: 
dxdt(11) =  lambda_tilde*(dxdt(3) + dxdt(4) + dxdt(5))...
            - beta_plus_tilde*PD1_0*x(10)*x(11) + beta_minus_tilde*PD1_0*x(12)/PDL1_0;

% Normalized to PD1_0
%(12) PD-1:PD-L1: 
dxdt(12) =  beta_plus_tilde*PDL1_0*x(10)*x(11) - beta_minus_tilde*x(12);
            
% Normalized to A_0
%(13) A (free drug): 
dxdt(13) = - alpha_plus_tilde*PD1_0*x(13)*x(10) + alpha_minus_tilde*x(14)...
           - delta_A_tilde*x(13);
            
% Normalized to A_0
%(14) A:PD-1: 
dxdt(14) = alpha_plus_tilde*PD1_0*x(13)*x(10) - alpha_minus_tilde*x(14);         
